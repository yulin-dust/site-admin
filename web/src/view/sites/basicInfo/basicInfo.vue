...
<template>
  ...
  <el-input v-model="searchInfo.cmsType" placeholder="搜索条件" />
</el-form-item>
            
            <el-form-item label="状态" prop="status">
    <el-tree-select v-model="searchInfo.status" placeholder="请选择状态" :data="statusOptions" style="width:100%" filterable :clearable="true" check-strictly ></el-tree-select>
</el-form-item>
    ...
    <div class="gva-table-box">
        <div class="gva-btn-list">
            ...
            <ExportTemplate  template-id="sites_BasicInfo" />
            <ExportExcel  template-id="sites_BasicInfo" filterDeleted/>
            <ImportExcel  template-id="sites_BasicInfo" @on-success="getTableData" />
        </div>
        <el-table
        ref="multipleTable"
        style="width: 100%"
        ...
        <el-table-column sortable align="left" label="日期" prop="CreatedAt" width="180">
            ...
            <el-table-column align="left" label="cms" prop="cmsType" width="120" />

            <el-table-column align="left" label="站点类型" prop="siteType" width="120" />
    ...
    {{ filterDict(scope.row.status,statusOptions) }}
        ...
        <el-table-column align="left" label="操作" fixed="right" :min-width="appStore.operateMinWith">
            ...
            <el-button  type="primary" link icon="edit" class="table-button" @click="updateBasicInfoFunc(scope.row)">编辑</el-button>
    ...
    {{ detailForm.apiToken }}
                    ...
                    <el-descriptions-item label="cms">
    {{ detailForm.cmsType }}
</el-descriptions-item>
    ...
    {{ detailForm.categoryIds }}
</el-descriptions-item>
                    <el-descriptions-item label="作者">
    {{ detailForm.authorIds }}
</el-descriptions-item>
...
const getTableData = async() => {
  const table = await getBasicInfoList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const onDelete = async() => {
  ElMessageBox.confirm('确定要删除吗?', '提示', {
    ...
    type: 'warning'
      ...
      const res = await deleteBasicInfoByIds({ IDs })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateBasicInfoFunc = async(row) => {
    const res = await findBasicInfo({ ID: row.ID })
    type.value = 'update'
    if (res.code === 0) {
...
// 删除行
const deleteBasicInfoFunc = async (row) => {
    const res = await deleteBasicInfo({ ID: row.ID })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
