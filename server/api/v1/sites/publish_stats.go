...
import (
	...
	sitesService "github.com/flipped-aurora/gin-vue-admin/server/service/sites"
...
// AddPublishCount 为指定 siteId 累加一次发布统计
// @Tags PublishStats
// @Summary 累加发布数（today_count+1，total_count+1，last_publish_time=now）
// @Accept application/json
// @Produce application/json
// @Param siteId query int true "站点ID"
...
func (publishStatsApi *PublishStatsApi) AddPublishCount(c *gin.Context) {
	ctx := c.Request.Context()

	siteIdStr := c.Query("siteId")
	if siteIdStr == "" {
		response.FailWithMessage("缺少 siteId 参数", c)
		return
	}
	siteId64, err := strconv.ParseInt(siteIdStr, 10, 32)
	if err != nil {
		response.FailWithMessage("siteId 参数非法: "+err.Error(), c)
		return
	}

	err = publishStatsService.AddPublishCount(ctx, int32(siteId64))
	if err != nil {
		if errors.Is(err, sitesService.ErrPublishStatsSiteNotFound) {
			response.FailWithMessage(err.Error(), c)
			return
		}
		global.GVA_LOG.Error("累加发布数失败!", zap.Error(err))
		response.FailWithMessage("累加失败:"+err.Error(), c)
		return
	}
	response.OkWithMessage("累加成功", c)
}
